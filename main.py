...
data_list_visitors= functions.get_data('visitors')

conn= functions.connect('postgres', 'toegang', 'Tom-1998', '127.0.0.1')
cursor=conn.cursor()

functions.drop_table(conn,'recommendation')
functions.drop_table(conn,'order_products')
functions.drop_table(conn,'stock')
functions.drop_table(conn,'products')
functions.drop_table(conn,'sessions')
functions.drop_table(conn,'visitors')
...
functions.create_table(conn,'visitors',qry_columns)
...
functions.create_table(conn,'order_products',qry_columns)
...
functions.create_table(conn,'recommendation',qry_columns)

functions.insertion_products(conn,data_list_products)
visitors_id_list=functions.insertion_visitors(conn,data_list_visitors)
