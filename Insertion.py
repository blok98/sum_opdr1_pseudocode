...
def insertion_visitors(conn,data_input,data_input_sessions):
    buids_list=[]
    count=0
    for datapoint in data_input:
        _id='Empty'
        ...
        insertion_visitors_buids(conn, _id, buids)

        sql = "INSERT INTO visitors VALUES('{}','{}','{}')".format(_id, latest_activity, latest_visit)
...
def insertion_sessions(conn,data_input):
    ...
    for datapoint in data_input:
        ...
        if 'buid' in datapoint:
            buid = datapoint['buid'][0]

        try:
            sql = "INSERT INTO sessions VALUES('{}','{}','{}','{}','{}','{}','{}')".format(_id,session_start,session_end,has_sale,cg_created,tg_created,buid)
            SQL.sqlexecute(conn, sql)
        except Exception as e:
            print(e)
            print('buid:',buid)
...
def insertion_buids(conn):
    pass


def insertion_visitors_buids(conn,visitor_id,buids):
    for buid in buids:
        qry="INSERT INTO visitors_buids VALUES('{}','{}')".format(visitor_id,buid)
        SQL.sqlexecute(conn,qry)